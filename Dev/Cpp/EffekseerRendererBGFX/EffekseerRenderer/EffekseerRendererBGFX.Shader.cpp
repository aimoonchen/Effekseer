					   ...
					   int32_t layoutCount)
{
	...
	auto shaderbgfx = shader.DownCast<Backend::Shader>();

	auto gd = graphicsDevice.DownCast<Backend::GraphicsDevice>();

	hr = gd->GetDevice()->CreateInputLayout(decl, layoutCount, shaderbgfx->GetVertexShaderData().data(), shaderbgfx->GetVertexShaderData().size(), &vertexDeclaration);
	...
	return new Shader(graphicsDevice, shaderbgfx, vertexDeclaration);
