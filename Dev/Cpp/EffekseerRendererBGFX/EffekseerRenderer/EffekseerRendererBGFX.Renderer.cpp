...
{
	if (m_backgroundBGFX == nullptr)
	{
		m_backgroundBGFX = graphicsDevice_->CreateTexture(background, nullptr, nullptr);
	}
	else
	{
		auto texture = static_cast<Backend::Texture*>(m_backgroundBGFX.Get());
		texture->Init(background, nullptr, nullptr);
	}

	EffekseerRenderer::Renderer::SetBackground((background) ? m_backgroundBGFX : nullptr);
}
